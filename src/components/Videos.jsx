...
const Videos = () => {
  ...
  const [videoSrc, setVideoSrc] = useState(videosArr[0].pathDirection);
  const [no, setno] = useState(1);
  const [desc, setdesc] = useState(videosArr[0].Description)
  return (
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
          <Heading>Sample Video {no + 1}</Heading>
          <Text>This is a sample video for testing and demo. In this video You will see the {desc}. </Text>
        </VStack>
      </VStack>
      <VStack
        w={['full', 'xl']}
      ...
      >
        {videosArr.map((item, index) => (
          <Button
            ...
            colorScheme={'teal'}
            onClick={() => {
              setVideoSrc(item.pathDirection);
              setno(index);
